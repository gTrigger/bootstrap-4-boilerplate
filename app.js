...
$(document).ready(function () {
    ...
    window.onload = function loadData() {
        ...
        for (var i = 0; i < returnArr.length; i++) {
            fillTableWithRows(returnArr[i].date, returnArr[i].name,
                            returnArr[i].projectID, returnArr[i].client,
                            returnArr[i].comment, returnArr[i].id)
        }
    };
        ...
        if (event.target.classList.contains('table-add-btn')) {
            ...
            containerTableInput.onclick = function (event) {
                ...
                if (event.target.classList.contains('form-check-btn')) {
                    ...
                    fillTableWithRows(form.elements.actionDate.value, form.elements.actionName.value,
                                        form.elements.actionProjectID.value, form.elements.actionClientName.value,
                                        form.elements.actionComment.value, obj.id);
                    containerTableInput.classList.toggle('hidden');
                }
